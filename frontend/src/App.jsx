...
const emptyProduct = { sku: "", name: "", description: "", unit_price: "", quantity_in_stock: "", reorder_level: "0" };
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="modal" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}>
        <header className="modal-header"><div><p className="eyebrow">CapsuleHub</p><h2>{title}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close">×</button></header>
        {children}
      </section>
    </div>
  );
...
function Dashboard({ summary, loading, setPage }) {
  const metrics = [
...
function ProductsPage({ products, onRefresh, notify }) {
  ...
  const openEdit = (product) => { setForm({ ...product, unit_price: String(product.unit_price), reorder_level: String(product.reorder_level) }); setModal("edit"); };
  ...
  const deleteProduct = async (product) => { if (!window.confirm(`Delete ${product.name}? Products used in orders cannot be deleted.`)) return; try { await api.deleteProduct(product.id); await onRefresh(); notify("success", "Product deleted."); } catch (error) { notify("error", error.message); } };
...
function CustomersPage({ customers, onRefresh, notify }) {
  const [search, setSearch] = useState(""); const [modal, setModal] = useState(null); const [form, setForm] = useState(emptyCustomer); const [saving, setSaving] = useState(false);
  ...
  const openEdit = (customer) => { setForm({ ...customer }); setModal("edit"); };
  const submit = async (event) => { event.preventDefault(); setSaving(true); try { modal === "create" ? await api.createCustomer(form) : await api.updateCustomer(form.id, { name: form.name, email: form.email, phone: form.phone || null, address: form.address || null }); setModal(null); await onRefresh(); notify("success", modal === "create" ? "Customer created." : "Customer updated."); } catch (error) { notify("error", error.message); } finally { setSaving(false); } };
  const deleteCustomer = async (customer) => { if (!window.confirm(`Delete ${customer.name}? Customers with orders cannot be deleted.`)) return; try { await api.deleteCustomer(customer.id); await onRefresh(); notify("success", "Customer deleted."); } catch (error) { notify("error", error.message); } };
...
function InventoryPage({ products, transactions, onRefresh, notify }) {
  ...
  const submit = async (event) => { event.preventDefault(); if (!productId || !change || !note.trim()) { notify("error", "Select a product, enter a non-zero adjustment, and add a reason."); return; } setSaving(true); try { await api.adjustInventory({ product_id: Number(productId), quantity_change: Number(change), note: note.trim() }); setProductId(""); setChange(""); setNote(""); await onRefresh(); notify("success", "Inventory adjusted and audit record created."); } catch (error) { notify("error", error.message); } finally { setSaving(false); } };
...
export default function App() {
  const [page, setPage] = useState("dashboard"); const [mobileOpen, setMobileOpen] = useState(false); const [data, setData] = useState({ summary: null, products: [], customers: [], orders: [], transactions: [] }); const [loading, setLoading] = useState(true); const [toast, setToast] = useState(null);
  const notify = (type, message) => { setToast({ type, message }); window.setTimeout(() => setToast(null), 4500); };
  const loadData = async () => { setLoading(true); try { const [summary, products, customers, orders, transactions] = await Promise.all([api.dashboard(), api.products(), api.customers(), api.orders(), api.transactions()]); setData({ summary, products, customers, orders, transactions }); } catch (error) { notify("error", `Could not load data: ${error.message}`); } finally { setLoading(false); } };
  useEffect(() => { loadData(); }, []);
  const changePage = (next) => { setPage(next); setMobileOpen(false); };
  let content; if (page === "products") content = <ProductsPage products={data.products} onRefresh={loadData} notify={notify} />; else if (page === "customers") content = <CustomersPage customers={data.customers} onRefresh={loadData} notify={notify} />; else if (page === "orders") content = <OrdersPage orders={data.orders} customers={data.customers} products={data.products} onRefresh={loadData} notify={notify} />; else if (page === "inventory") content = <InventoryPage products={data.products} transactions={data.transactions} onRefresh={loadData} notify={notify} />; else content = <Dashboard summary={data.summary} loading={loading} setPage={changePage} />;
  return <div className="app-shell"><aside className={`sidebar ${mobileOpen ? "open" : ""}`}><div className="brand"><div className="brand-mark">C</div><div><strong>CapsuleHub</strong><span>Operations console</span></div></div><nav>{NAV_ITEMS.map(([id, label, icon]) => <button key={id} className={page === id ? "nav-item active" : "nav-item"} onClick={() => changePage(id)}><span>{icon}</span>{label}</button>)}</nav><div className="sidebar-footer"><span className="live-dot"></span><span>API connected</span></div></aside><main className="main"><header className="topbar"><button className="menu-button" onClick={() => setMobileOpen(!mobileOpen)}>☰</button><div><span className="workspace-label">Workspace</span><strong>Inventory management</strong></div><div className="topbar-actions"><button className="refresh-button" onClick={loadData} disabled={loading}>{loading ? "Refreshing…" : "↻ Refresh"}</button><div className="avatar">AS</div></div></header>{content}</main><Toast toast={toast} onClose={() => setToast(null)} /></div>;
}
